...
LOG = logging.getLogger('app.beba.nativemonitoring')
...
class NativeMonitoring(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):
		...
		self.datapaths = {}
		self.nbRules = 0
		# self.IPsrc = {} # Dictionary IP source: IP, counter
		# self.IPdst = {} # Dictionary IP dest: IP, counter
		self.step = 1
		self.t0 = self.t1 = self.timeDiff = datetime.now()
		self.ReactionTimes = {} # Dictionary of reaction times: step, reaction time
		self.repSize = 0
		self.ReplySizes = {}  # Dictionary of reply sizes: step, reply size
		self.outputFile = '/home/tai/beba-ctrl/ryu/app/beba/results/results.txt'
	...
	def flow_stats_reply_handler(self, ev):
		...
		self.ReactionTimes[self.step] = self.timeDiff.total_seconds()
		...
		if len(ev.msg.body) >= 1:
			self.repSize = (len(ev.msg.body)-1)*112+72+82
		else:
			self.repSize = 82 # empty FlowStats reply
		self.ReplySizes[self.step] = self.repSize
		flows = []
		LOG.info("Parsing flow stat reply (%s bytes)...", self.repSize)
		...
		LOG.info('ReactionTimes: %s', self.ReactionTimes)
		# Step | ReactionTime | IpFlowEntriesRep | FlowStatRepSize | TotalFlowEntries | ReqFrequency
		#LOG.info('%s %s %s %s %s %s', self.step, self.timeDiff.total_seconds(), len(flows), self.repSize, self.nbRules, self.monitor_interval)
		result = str(self.step) + ' ' + str(self.timeDiff.total_seconds()) + ' ' + str(len(flows)) + ' ' + str(self.repSize) + ' ' + str(self.nbRules) + ' ' + str(self.monitor_interval)
		f=open(self.outputFile, 'ab')
		f.write(result+'\n')
		f.close()
		self.step+=1
	...
	# 	tables = []
	# 	LOG.info("Parsing table stat reply...")
	# 	for stat in ev.msg.body:
	# 		tables.append('table_id=%d active_count=%d lookup_count=%d '
	# 			' matched_count=%d' %
	# 			(stat.table_id, stat.active_count,
	# 				stat.lookup_count, stat.matched_count))
	# 	LOG.info('TableStats: %s', tables)
